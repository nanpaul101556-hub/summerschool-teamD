 ...
 *
 * 굵은 선 하나가 이 화면의 주인공이다. 준공 1989 년에 100 에서 출발해 계속 내려오다가
 * 확인된 공사가 있는 2025 년에 한 번 되돌아간다 — 지나온 구간에서 층이 통째로
 * 되돌아갔다고 말할 수 있는 해는 그 하나뿐이다 (clocks.js COMPOSITE_PAST).
 * 지금(2026)에서 두 갈래로 갈린다 — 손대지 않으면 계속 내려가고,
 * 판정대로 손대면 2031·2034 에서 두 번 더 반등한다.
 *
...
import {
  ...
  HORIZONS, HORIZON_END, LAST, LAYER_LINES, MEET_YEARS, NOW, residual, span,
} from '../lib/clocks'
...
export default function ValueCurve({ onOpen }) {
  const { t, tx } = useLang()

  const none = COMPOSITE.find((c) => c.key === 'none')
  const plan = COMPOSITE.find((c) => c.key === 'plan')
  const nowV = COMPOSITE_PAST.at(-1).v
  /** 확인된 공사가 만든 유일한 과거 반등 — 그 꼭대기 */
  const workV = COMPOSITE_PAST.filter((p) => p.y === LAST).at(-1).v
  ...
  return (
    <figure className="vc">
      ...
      {/* 곡선 전체가 버튼이다 — 눌러 열면 같은 이야기를 건물 자체로 한 번 더 한다 */}
      <div className="vc-fig">
        <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={t('vc.title')}>
          ...
          <circle cx={sx(LAST)} cy={sy(workV)} r="3.4" className="vc-cd past" />
        ...
        {onOpen && (
          <button type="button" className="vc-open" onClick={onOpen}>
            <span>
              <i aria-hidden="true" />
              {t('lf.open')}
            </span>
